...
struct Student {
    char nisn[20];
    char name[50];
    char gender[10];  // "Laki-laki" atau "Perempuan"
    int kip;  // 1 untuk Ya, 0 untuk Tidak
};
...
void deleteStudent() {
    ...
    if (students[index].kip) kip_count--;
...
void sortStudents() {
    ...
    switch (choice) {
        case 1:
            qsort(students, student_count, sizeof(struct Student), compareNISN);
            break;
        case 2:
            qsort(students, student_count, sizeof(struct Student), compareName);
            break;
        case 3:
            qsort(students, student_count, sizeof(struct Student), compareKIP);
            break;
        default:
